...
export class VectorIndex {
  ...
  search(query: Float32Array, topK: number): Array<{ id: string; similarity: number }> {
  ...
  loadFromRecords(records: Array<{ id: string; embedding: Float32Array }>): void {
    this.ids = [];
    this.count = 0;
    this.vectors = new Float32Array(records.length * DIMS);

    for (const record of records) {
      if (record.embedding.length !== DIMS) continue;
