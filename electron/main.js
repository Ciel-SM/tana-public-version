...
// ── Mind Palace state ──────────────────────────────────────────────────────
let mindPalaceStorage = null;
let mindPalaceIndex = null;
...
function parseLiteModeOverride() {
...
function createWindow() {
    ...
    try {
        ...
        trayIcon = nativeImage.createFromDataURL('data:image/png;base64,');
    }
...
ipcMain.handle('capture-screenshot-jpeg', async (_event, quality) => {
    try {
        const thumbnail = await captureScreen();
        const jpegQuality = Number.isFinite(quality)
            ? Math.max(30, Math.min(90, Math.round(quality)))
            : 75;
...
ipcMain.handle('capture-region', async (_event, x, y, w, h, viewportWidth, viewportHeight) => {
    try {
        const primary = screen.getPrimaryDisplay();
        const thumbnail = await captureScreen();
        if (!thumbnail)
        ...
        const cropped = thumbnail.crop(mapOverlayRegionToCaptureRegion(primary, overlayContentBounds, overlayViewportSize, thumbnail.getSize(), { x, y, width: w, height: h }));
...
ipcMain.handle('set-ignore-mouse-events', (_event, ignore, opts) => {
    if (ignore) {
...
ipcMain.handle('mind-palace:store-memory', async (_event, payload) => {
...
ipcMain.handle('mind-palace:search', async (_event, embeddingArray, topK) => {
    if (!mindPalaceStorage || !mindPalaceIndex) {
        throw new Error('Mind Palace not initialized');
    }
...
ipcMain.handle('mind-palace:search-keyword', async (_event, query, limit) => {
    if (!mindPalaceStorage)
        throw new Error('Mind Palace not initialized');
    return mindPalaceStorage.searchByKeyword(query, limit);
});
ipcMain.handle('mind-palace:get-memory', async (_event, id) => {
    if (!mindPalaceStorage)
        throw new Error('Mind Palace not initialized');
    return mindPalaceStorage.getMemory(id);
});
ipcMain.handle('mind-palace:load-image', async (_event, relativePath) => {
    if (!mindPalaceStorage)
        throw new Error('Mind Palace not initialized');
...
ipcMain.handle('mind-palace:get-recent', async (_event, limit) => {
    if (!mindPalaceStorage)
        throw new Error('Mind Palace not initialized');
...
ipcMain.handle('agent:run-applescript', async (_event, script) => {
...
ipcMain.handle('agent:open-app', async (_event, appName) => {
    return openApp(appName);
});
ipcMain.handle('agent:type-text', async (_event, text) => {
    return typeText(text);
});
ipcMain.handle('agent:press-keys', async (_event, keys) => {
    return pressKeys(keys);
});
ipcMain.handle('agent:shell-exec', async (_event, command, timeoutMs) => {
    return shellExec(command, timeoutMs);
});
ipcMain.handle('agent:web-search', async (_event, query) => {
